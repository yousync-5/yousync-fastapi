...
유튜브 링크 분석, 배우 추출, 얼굴 인식 등의 기능을 제공합니다.
"""
...
class YouTubeAnalysisRequest(BaseModel):
    """유튜브 분석 요청 스키마"""
    youtube_url: HttpUrl
    movie_title: Optional[str] = None  # 사용자가 직접 입력한 영화 제목
...
@router.post("/movie/actors", response_model=ActorListResponse)
async def get_movie_actors(request: YouTubeAnalysisRequest):
    """
    영화 제목 또는 유튜브 메타데이터에서 출연 배우 목록을 가져옵니다.
    
    - **youtube_url**: 유튜브 비디오 URL (메타데이터 추출용)
    - **movie_title**: 직접 입력한 영화 제목 (선택사항, 우선적으로 사용)
    """
    try:
        url_str = str(request.youtube_url)
        
        # 사용자가 영화 제목을 직접 입력한 경우
        if request.movie_title:
            logger.info(f"사용자 입력 제목 사용: {request.movie_title}")
            actors, matched_title = actor_service.get_cast_list_by_title(request.movie_title)
        else:
            # 유튜브 메타데이터에서 영화 정보 추출
            logger.info("유튜브 메타데이터에서 영화 정보 추출")
            metadata = youtube_service.extract_metadata(url_str)
            actors, matched_title = actor_service.get_cast_list_from_metadata(metadata)
        
        if matched_title == "unknown":
            raise HTTPException(status_code=404, detail="영화 정보를 찾을 수 없습니다.")
        elif matched_title == "excluded_genre":
            raise HTTPException(status_code=400, detail="다큐멘터리나 토크쇼는 지원하지 않습니다.")
        elif matched_title == "error":
            raise HTTPException(status_code=500, detail="영화 정보 조회 중 오류가 발생했습니다.")
        ...
        raise HTTPException(status_code=500, detail=f"배우 정보 추출 중 오류가 발생했습니다: {str(e)}")
...
async def start_full_analysis(
    ...
    1. 메타데이터 추출
    2. 영화 정보 및 배우 목록 조회
    3. 배우 이미지 크롤링 및 인코딩 (백그라운드)
    4. 음성 추출 및 자막 생성 (백그라운드)
    
    - **youtube_url**: 분석할 유튜브 비디오 URL
    - **movie_title**: 직접 입력한 영화 제목 (선택사항)
    """
    try:
        url_str = str(request.youtube_url)
        ...
        if matched_title in ["unknown", "excluded_genre", "error"]:
            return AnalysisStatusResponse(
                status="failed",
                message="영화 정보를 찾을 수 없거나 지원하지 않는 콘텐츠입니다.",
                data={"metadata": metadata}
            )
        
        # 3. 백그라운드 작업 시작 (추후 구현)
        # background_tasks.add_task(process_actor_images, actors)
        # background_tasks.add_task(process_audio_extraction, url_str)
...
# 추후 추가될 엔드포인트들:
# - /analysis/status/{job_id} - 작업 상태 조회
